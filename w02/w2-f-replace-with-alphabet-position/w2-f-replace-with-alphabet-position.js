...
function alphabetPosition(text) {
  let unicodeStartValue = 96;
  let numberedString = text
    .split(' ')
    .join('')
    .split('')
    .map((letter) => {
      let numInAlphabet = letter.toLowerCase().charCodeAt() - unicodeStartValue;
      if (numInAlphabet > 0) return numInAlphabet;
    })
    .join(' ');
